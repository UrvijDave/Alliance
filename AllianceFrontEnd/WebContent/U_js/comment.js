/**
 * 
 * jquery.charcounter.js version 1.2 requires jQuery version 1.2 or higher
 * Copyright (c) 2007 Tom Deater (http://www.tomdeater.com) Licensed under the
...
(function($) {
	 ...
	 * object usage: $("#myTextArea").charCounter(max, settings);
	 */
	...
	$.fn.charCounter = function(max, settings) {
		...
		function count(el, container) {
			...
			if (settings.delay > 0) {
				...
				container.html(settings.format.replace(/%1/,
						(max - el.val().length)));
			}
		}
		;
		...
		return this.each(function() {
			...
			if (!settings.container.match(/^<.+>$/)) {
				...
				// append element to hold counter message (clean up old element
				// first)
				$(this).next("." + settings.classname).remove();
				container = $(settings.container).insertAfter(this).addClass(
						settings.classname);
			}
